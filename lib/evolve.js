...
(function() {
  ...
  exports.clone = clone = function(opts) {
    var e, e1, e2, globals, i, inlinePaths, item, items, k, mutateBranch, mutateTree, mutations, name, options, reservoir_ast, resolve, v, work, _i, _len, _ref;
    options = {
      src: "",
      ratio: 0.01,
      iterations: 2,
        ...
        if (isArray(node)) {
          if (node[0] === 'call') {
            if ((_ref1 = "" + node[1]) === 'dot,name,evolve,mutable' || _ref1 === 'name,mutable') {
              found = true;
              return node[2] = mutateBranch(copy(node[2]));
            } else {
              _results = [];
              for (_j = 0, _len1 = node.length; _j < _len1; _j++) {
                n = node[_j];
